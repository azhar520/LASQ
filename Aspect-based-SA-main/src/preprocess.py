...
class Preprocessor:
    def __init__(self, config):
        ...
        with open(f"./data/dataset/jsons_{config.lang}/posDic.json", "r", encoding="utf-8") as f:
    ...
    def check_text(self, tokenized_text, source_text):
        ...
        t0 = tokenized_text.replace('##', '').replace('▁', '').replace(self.config.unk, '').lower()
        t1 = source_text.replace(' ', '').lower().replace("，", ",").replace("？", "?").replace("（", "(").replace("）", ")").replace("：", ":").replace("！", "!")
    ...
    def parse_dialogue(self, dialogue, mode):
        ...
        pos = dialogue['pos']
        pos_id = [[self.posDic[p_item] for p_item in p] for p in pos]
        # align_index_with_list: align the index of the original elements according to the tokenization results
        new_sentences, pieces2words, adj2word = self.align_index_with_list(sentences, words)

        new_pos_id = []
        for i, piece2word in enumerate(adj2word):
            temp = []
            for j, word in enumerate(piece2word):
                temp.append(pos_id[i][word])
            new_pos_id.append(temp)
        new_pos_id = [w for line in pos_id for w in line]

        new_adj2word = []
        for i, line in enumerate(adj2word):
            if i > 0:
                delt = new_adj2word[-1] + 1
                for w in line:
                    new_adj2word.append(w + delt)
            else:
                new_adj2word.extend(line)
        # print(words[1])
        # print(new_sentences[1], len(new_sentences[1]))
        # print(adj2word[1], len(adj2word[1]))
        # assert len(new_sentences[1]) == len(adj2word[1])
        # assert len(new_sentences[1]) == len(new_pos_id[1])
        n = sum([len(item) for item in adj])
        new_adj = torch.zeros(n, n)
        for ind, adj_item in enumerate(adj):
            for i, hang in enumerate(adj_item):
                for j, lie in enumerate(adj_item[i]):
                    if ind > 0:
                        delt = sum([len(item) for item in adj[:ind]])
                        for w in line:
                            if adj_item[i][j]:
                                new_adj[i+delt,j+delt] = adj_item[i][j]
                    else:
                        if adj_item[i][j]: 
                            new_adj[i,j] = adj_item[i][j]
        new_adj = new_adj.tolist()
        ...
        dialogue['adj2word'] = new_adj2word
        dialogue['adj'] = new_adj
        dialogue['pos_id'] = new_pos_id
    ...
    def align_index_with_list(self, sentences, words):
        """_summary_
        ...
        for i, sentence in enumerate(sentences):
            ...
            for word in cur_line:
                newW.append(word.replace("##", "").replace("▁", "").lower())
                for index, value in enumerate(words[i]):
                    if value.lower() == "".join(newW):
                        if index == temp[-1] + 1:
                            temp.extend([index] * len(newW))
                            newW = []
                            break
            adj2word.append(temp[1:])
        return all_pieces, pieces2word, adj2word
    ...
    def transform2indices(self, dataset, mode='train'):
        ...
        for document in dataset:
            sentences, speakers, replies, pieces2words, adj, adj2word, pos_id = [document[w] for w in ['sentences', 'speakers', 'replies', 'pieces2words', 'adj', 'adj2word', 'pos_id']]
            if True:
                triplets, targets, aspects, opinions = [document[w] for w in ['triplets', 'targets', 'aspects', 'opinions']]
            doc_id = document['doc_id']
            old_replies = deepcopy(replies)

            sentence_length = [len(w) + 1 for w in sentences]
            sentence_length[-1] += 1
            ...
            token2speaker = [[11] + [w] * len(z) for w, z in zip(speakers, sentences)]
            token2speaker = [w for line in token2speaker for w in line] + [11]
            ...
            for i in range(len(sentence_length)):
                for j in range(sentence_length[i]):
                    ...
                    if j == 0 or (i == len(sentence_length) - 1 and j == sentence_length[i] - 1):
                        new2old[len(new2old)] = -1 
                    else:
                        new2old[len(new2old)] = cur_len
                        cur_len += 1

            tokens = [[self.config.cls] + w for w in sentences]
            tokens[-1] += [self.config.sep]
            ...
            sentence_end = [i - 1 for i, w in enumerate(flatten_tokens) if w == self.config.cls and i != 0]
            sentence_end += [len(flatten_tokens) - 2]
            ...
            input_ids = [w for line in tokens for w in line]
            ...
            if True:
                targets = [(s + token2sentid[s] + 1, e + token2sentid[s]) for s, e, t in targets]
                aspects = [(s + token2sentid[s] + 1, e + token2sentid[s]) for s, e, t in aspects]
                opinions = [(s + token2sentid[s] + 1, e + token2sentid[s]) for s, e, t, p in opinions]
                ...
                for t_s, t_e, a_s, a_e, o_s, o_e, polarity, t_t, a_t, o_t in triplets:
                    new_index = lambda start, end : (-1, -1) if start == -1 else (start + token2sentid[start] + 1, end + token2sentid[start])
            ...
            res.append((doc_id, input_ids, input_masks, input_segments, sentence_length, nsentence_ids, utterance_index, token_index, 
                        thread_length, token2speaker, reply_mask, speaker_masks, thread_masks, pieces2words, adj, adj2word, pos_id, new2old, 
                        new_triplets, pairs, entity_lists, relation_lists, speakers, old_replies))
        return res
