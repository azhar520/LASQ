...
class MyDataLoader:
    ...
    def collate_fn(self, lst):
        doc_id, input_ids, input_masks, input_segments, sentence_length, token2sents, utterance_index, token_index, thread_length, token2speaker, reply_mask, speaker_mask, thread_mask, pieces2words, adj, adj2word, pos_id, new2old, triplets, pairs, entity_list, rel_list, speakers, replies = zip(*lst)
        ...
        # input_ids, input_masks, input_segments = map(padding, [input_ids, input_masks, input_segments])print(o
        max_lens = max([len(w) for w in input_ids])
        input_ids = [w + [0] * (max_lens - len(w)) for w in input_ids]
        pos_id = [w + [0] * (max_lens - len(w)) for w in pos_id]
        input_masks = [w + [0] * (max_lens - len(w)) for w in input_masks]
        input_segments = [w + [0] * (max_lens - len(w)) for w in input_segments] 
        padding_adj  = []
        # print(len(adj2word))
        for sents_id, sent in enumerate(adj2word):
            # for sent_id, sent in enumerate(sentences):
                l = len(sent) - 2
                padding = torch.zeros(max_lens, max_lens)
                for i in range(1, l+1):
                    for j in range(1, l+1):
                        # padding[i, j] = adj[sents_id][sent_id][sent[i-1]][sent[j-1]]
                        padding[i, j] = adj[sents_id][sent[i-1]][sent[j-1]]
                # torch.set_printoptions(profile="full")
                padding_adj.append(padding.tolist())
        # exit()
        ...
        res = {
            "doc_id": doc_id,
			"input_ids": input_ids, "input_masks": input_masks, "adj":padding_adj, "pos_id": pos_id, "input_segments": input_segments,
